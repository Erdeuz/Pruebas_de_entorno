...
pygame.display.set_caption("Juego de Sigilo de tu vieja")
...
enemigo_velocidades = [5, 6, 5, 7, 12, 55, 99]

# Configuración de la meta
meta = pygame.Rect(520, ALTO // 2 - 10, 20, 20)
...
while ejecutando:
    ...
    # Movimiento de la meta para escapar
    if meta.x >= jugador_x:
        meta.x += meta_velocidad
    else:
        meta.x -= meta_velocidad

    if meta.y >= jugador_y:
        meta.y += meta_velocidad
    else:
        meta.y -= meta_velocidad

    # Asegurarse de que la meta permanezca dentro de los límites
    if meta.x < 0:
        meta.x = 0
    elif meta.x > ANCHO - meta.width:
        meta.x = ANCHO - meta.width

    if meta.y < 0:
        meta.y = 0
    elif meta.y > ALTO - meta.height:
        meta.y = ALTO - meta.height
